...
!****************************************************************************
	subroutine hw9_newtondd(n,a,b)
	  integer n,i,j
	  real*8 a,b,x(0:n),f(0:n,0:n),h,y,p,maxerr,err,t1,t2,errx
	  h=(b-a)/n
	  do i=0,n
	    x(i)=a+i*h
	    f(i,0)=fun(x(i))
...
!	    y=a+0.1+i*h
	  ...
	  write(11,"('a=',F6.3),2X,'b=',F6.3,4X,'n=',I3")a,b,n
	  ...
	  do i=0,999
		...
		t2=pn(n,x,f,(a+i*h))
		err=dabs((t2-t1)/t1)
		write(12,"(I4,2X,3E20.12)")i,t1,t2,err
		if(maxerr .le. err)then
		  maxerr=err
...
!****************************************************************************
	function pn(n,x,f,y)
	  integer n,i,j
	  real*8 x(0:n),f(0:n,0:n),y,tt,pn
	  pn=f(0,0)
	  do i=1,n
	    tt=f(i,i)
	    do j=0,i-1
